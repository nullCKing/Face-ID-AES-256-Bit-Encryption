...
void Decrypt()
{
	...
	std::vector<char> unhexPlaintext;
	// Copy the state array to output
	...
	for (i = 0; i < 4; i++)
	{
		for (j = 0; j < 4; j++)
		{
			unhexPlaintext.push_back(state[j][i]);
			std::cout << state[j][i];
		}
	}
	std::cout << "\n\nDecrypted plaintext: ";
	...
	plainString.resize(unhexPlaintext.size());

	std::copy(unhexPlaintext.begin(), unhexPlaintext.end(), plainString.begin());

	int len = plainString.length();
	std::string newString;
	for (int i = 0; i < len; i += 2)
	{
		std::string byte = plainString.substr(i, 2);
...
int main()
{
	...
	std::cout << "Program start time: " << startTime << "\n\n";

	std::string Source = "f619db710a02036d9b6caf5d3f61c773";
	...
	std::ifstream file("text.txt", std::ios::binary | std::ios::in);
	...
	if (fileSize < 16) {
		buffer.resize(16);
		//std::cout << "File was too small. Resized to 16.";
	}
	else {
	...
	std::string hexCrypt;
	for (unsigned char c : buffer) {
		hexCrypt += "0123456789ABCDEF"[c >> 4];
		hexCrypt += "0123456789ABCDEF"[c & 15];
	}

	if (obj.SecurityCheck(startTime) == true)
	{
		...
		std::cout << "Facial recognition check failed.";
